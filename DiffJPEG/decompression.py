...
import torch
# Local
import DiffJPEG.jpeg_utils as jpeg_utils
...
def y_dequantize(image, factor=1):
    ...
    """
    return image * (jpeg_utils.y_table * factor)
...
def c_dequantize(image, factor=1):
    ...
    """
    return image * (jpeg_utils.c_table * factor)
...
def idct_8x8(image):
    ...
    """
    alpha = np.array([1. / np.sqrt(2)] + [1] * 7)
    alpha = torch.tensor(np.outer(alpha, alpha))
    image = image * alpha

    tensor = np.zeros((8, 8, 8, 8), dtype=np.float64)
...
def decompress_jpeg(y, cb, cr, height, width, rounding=torch.round, factor=1):
    ...
    image = torch.min(255*torch.ones_like(image),
                      torch.max(torch.zeros_like(image), image)).float()
    return image/255